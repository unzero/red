...
use crate::web::errors::RedHttpError;
...
pub async fn index(
    ...
    match identity {
        Some(_) => common::redirect("/red"),
        _ => common::render_template("red/index.html", crate::context!({"errors": {}}), templates),
    }
}

pub async fn red_login(
    form: actix_web::web::Form<RedLogin>,
...
pub async fn home(
    ...
    let files = red_users
        .lock()
        .map_err(|_e| RedHttpError::Default)?
        .get_mut(&uuid_str)
        .ok_or(RedHttpError::Default)?
        .get_files()
        .map_err(|_e| RedHttpError::Default)?;
...
pub async fn open_file(
    target: actix_web::web::Json<Redfile>,
    identity: Option<Identity>,
    red_users: actix_web::web::Data<crate::RedUsers>,
) -> Result<HttpResponse, RedHttpError> {
    let uuid_str = session::validate_session(identity)?;
    let filename = red_users
        .lock()
        .unwrap()
        .get_mut(&uuid_str)
        .unwrap()
        .query_file_uuid(target.get_uuid())
        .map_err(|_e| RedHttpError::Default)?;
    let file_content = red_users
        .lock()
        .unwrap()
        .get_mut(&uuid_str)
        .unwrap()
        .read_file_content(target.get_uuid())
        .map_err(|_e| RedHttpError::Default)?;
...
pub async fn save_file(
    ...
    let content = target
        .get_file_content()
        .ok_or_else(|| RedHttpError::Default)?;
    let result = red_users
        .lock()
        .unwrap()
        .get_mut(&user_uuid)
        .unwrap()
        .save_file(target.get_uuid(), content)
        .map_err(|_e| RedHttpError::Default)?;
